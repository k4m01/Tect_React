import React from "react";

export default function FromText(){
    const textPwsd1 = React.useRef()
    const textPwsd2 = React.useRef()

    const onBlurPassword = () => {
        let pswd = textPwsd1.current.value
        if(pswd !== '' && !pswd.match(/^[0-9a-zA-Z]+$/)){
            textPwsd1.current.value = ''
            alert('ต้องเป็น 0-9 หรือ a-z หรือ A-Z เท่านั้น')
        }
    }

    const onBlurPassword2 = () => {
        if(textPwsd1.current.value !== textPwsd2.current.value){
            textPwsd2.current.value = ''
            alert('รหัสผ่านทั้งสองช่องไม่ตรงกัน')
        }
    }

    return(
        <div className="mt-4 mx-auto p-3 rounded" style = {{ width: "400px", background: "#cee"}}>
            <form>
                <div className="form-group mb-2">
                    <label htmlFor="login">ชื่อผู้ใช้ (login)</label> 
                    <input type="name" id="login" name="login" maxLength="20"
                        className="form-control form-control-sm"></input>
                </div>
                <div className="form-group mb-2">
                    <label htmlFor="pswd">รหัสผ่าน</label> 
                    <input type="password" id="pswd" name="login" maxLength="10"
                        className="form-control form-control-sm"
                        ref={textPwsd1} onBlur={onBlurPassword}></input>
                </div>
                <div className="form-group mb-2">
                    <label htmlFor="pswd">ใส่รหัสผ่านซ้ำ</label> 
                    <input type="password" id="pswd" name="login" maxLength="10"
                        className="form-control form-control-sm"
                        ref={textPwsd2} onBlur={onBlurPassword2}></input>
                </div>
                <div className="form-group mb-4">
                    <label htmlFor="memo">บันทึกช่วยจำ</label> 
                    <textarea id="memo" name="memo" rows="2" maxLength="200"
                        className="form-control form-control-sm">
                    </textarea>
                </div>
                <div className="text-center">
                    <button className="btn btn-sm px4 btn-primary">OK</button>
                </div>
            </form>

        </div>    
    )


}